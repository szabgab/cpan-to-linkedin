...
Without LinkedIn authentication the script can only do a public profile lookup,
so `connection_status` is reported as `unknown`. To inspect connection status,
pass a raw LinkedIn `Cookie` header either with `--linkedin-cookie`,
